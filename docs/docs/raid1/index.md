...
Photos存储池创建之后, 可以从**设备** 按钮点击进入, 看到已经创建的镜像与设备ID, 未来如果换更大的硬盘, 可以依次在此分离再拆换!
![ZFS RAID1的设备](../images/storage/raid1-devices.jpg)

### 4. 创建ZFS 共享

RAID1 存储池创建完成后，需要创建 **ZFS 共享** 才能通过网络（如 SMB/Samba）访问其中的数据。ZFS 共享将存储池中的一个数据集（Dataset）共享给局域网中的用户，方便在 Windows、macOS、Linux 等设备上读写文件。

![ZFS 共享](../images/storage/zfs-share.jpg)

上图是创建 ZFS 共享的界面，各字段含义如下：

| 字段/按钮 | 说明 |
|-----------|------|
| **选择数据集** | 选择要共享的 ZFS 数据集，例如 `Photos` |
| **所有者** | 设置该共享数据集的所有者用户，图中为 `joe` |
| **配额** | 限制该共享可使用的最大空间，`none` 表示不限制 |
| **G** | 配额单位，此处为 GB（千兆字节） |

创建成功后，即可通过 Samba 访问该共享，例如：
